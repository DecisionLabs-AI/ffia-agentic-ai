...
_FUEL_ALIASES: dict[str, str] = {
    ...
    "b20":          "diesel",
    # Hi Diesel S — ALL generic diesel queries resolve here (single default)
    "diesel":       "hi diesel s",
    "ดีเซล":        "hi diesel s",
    "ไฮดีเซล":      "hi diesel s",
...
def get_oil_price_from_bangchak(fuel_type: str = "hi diesel s") -> dict:
    ...
    Args:
        fuel_type: English name, Thai alias, or abbreviation.
                   Defaults to "hi diesel s" (Hi Diesel S — single FFIA default).
                   Examples: "hi diesel s", "b20", "g95", "เบนซิน 91", "e20".
...
def oil_price_tool(fuel_type: str = "hi diesel s") -> str:
    ...
    Defaults to Hi Diesel S — the single FFIA default for ALL generic diesel queries.
    Only use "b20" when the user explicitly asks for standard diesel / B20.

    Supported fuel_type values:
      Hi Diesel S (default): "hi diesel s", "diesel", "ดีเซล", "ไฮดีเซล"
      Diesel B20 (explicit only): "b20"
